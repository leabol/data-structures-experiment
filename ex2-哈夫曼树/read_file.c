...
void to_utf8(int decode, char *utf_8)
{
        ...
        int len = 0;
            if (decode <= 0x7F){// 7bit
                utf8[0] = decode;
                len = 1;
            }else if (decode <= 0x7FF){// 5 + 6 == 11bit
                utf8[0] = 0xC0 | (decode >> 6);
                utf8[1] = 0x80 | (decode & 0x3F);
                len = 2;
            }else if (decode <= 0xFFFF){// 4 + 6 + 6 == 16bit
                utf8[0] = 0xE0 | (decode >> 12);
                utf8[1] = 0x80 | ((decode >> 6) & 0x3F);
                utf8[2] = 0x80 | (decode & 0x3F);
                len = 3;
            }else{// 3 + 6 + 6 + 6 == 21bit
                utf8[0] = 0xF0 | (decode >> 18);
